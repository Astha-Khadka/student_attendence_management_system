...
public class EditProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Always use the existing photoPath if no new photo is uploaded
        String existingPhotoPath = null;
        if (user instanceof Teacher) {
            existingPhotoPath = ((Teacher) user).getPhotoPath();
        }
        if (photoPath != null && !photoPath.isEmpty()) {
            updatedTeacher.setPhotoPath(photoPath);
        } else {
            updatedTeacher.setPhotoPath(existingPhotoPath);
        }
        photoPath = updatedTeacher.getPhotoPath(); // Always pass the correct value to DAO
