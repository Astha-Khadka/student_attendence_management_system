...
public class StudentDAO {
    ...
    public Student getStudentData(int userId) throws SQLException {
        ...
        String query = "SELECT s.*, u.username, u.email FROM student s JOIN users u ON s.user_id = u.id WHERE s.user_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    // Assuming Student model has fields for username and email from users table
                    student.setUsername(rs.getString("username"));
                    student.setEmail(rs.getString("email"));
                }
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error fetching student data for user ID: " + userId, e);
    ...
    public boolean updateStudent(Student student) throws SQLException {
        ...
        try {
            ...
            // Update users table (username and email only)
            String sqlUser = "UPDATE users SET username = ?, email = ? WHERE id = ?";
            pstmtUser = conn.prepareStatement(sqlUser);
            pstmtUser.setString(1, student.getUsername());
            pstmtUser.setString(2, student.getEmail());
            pstmtUser.setInt(3, student.getId());
            pstmtUser.executeUpdate();

            // Update student table (classname and photo_path)
            String sqlStudent = "UPDATE student SET classname = ?, photo_path = ? WHERE user_id = ?";
            pstmtStudent = conn.prepareStatement(sqlStudent);
            pstmtStudent.setString(1, student.getClassName());
            pstmtStudent.setString(2, student.getPhotoPath());
            pstmtStudent.setInt(3, student.getId());
    ...
    public List<Attendance> getStudentAttendanceRecords(int userId) throws SQLException {
        List<Attendance> attendanceRecords = new ArrayList<>();
        // Corrected table name to 'attendance' as per schema
        String query = "SELECT * FROM attendance WHERE User_ID = ? ORDER BY Date DESC";
        try (Connection conn = DatabaseConnection.getConnection();
                PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setInt(1, userId);
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    record.setId(rs.getInt("AttendenceID"));
                    record.setUserId(rs.getInt("User_ID"));
                    record.setDate(rs.getDate("Date"));
                    record.setFaculty(rs.getString("Faculty"));
                    record.setStatus(rs.getString("Status"));
                    record.setDay(rs.getString("Day"));
                    attendanceRecords.add(record);
                }
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error fetching student attendance records for user ID: " + userId, e);
            throw e; // Re-throw the exception after logging
        }
        return attendanceRecords;
    }

    public List<Notification> getStudentNotifications() throws SQLException {
